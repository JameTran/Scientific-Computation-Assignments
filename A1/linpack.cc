...
#ifdef SP
#define PREC "Single"
#define DIGITS std::numeric_limits<REAL>::digits10
typedef float REAL;
#endif
...
#ifdef LDP
// INSERT THE STUFF NEEDED FOR long double here.
...
template <typename T>
double ToDouble(T &d) {
#if defined(QD_REAL) || defined(DD_REAL)
  return to_double(d);
#else
  return double(d);
#endif
 ...
 */
    ...
    printf("    Reps Time(s) DGEFA   DGESL  OVERHEAD    MFLOPS     GFLOPS\n");
    printf("-------------------------------------------------------------\n");
    nreps = 1;
...

{
  REAL * a, *b;
  REAL   norma;
  double kflops, mflops, gflops, ops;
  double tdgesl, t1, tdgefa, totalt, toverhead;
  int *  ipvt, n, info, lda;
  long   i, arsize2d;
  ...
  mflops    = 2. * nreps * ops / (1000000. * (tdgefa + tdgesl));
  gflops    = 2. * nreps * ops / (1000000000. * (tdgefa + tdgesl));
  ...
  printf("%8ld %6.2f %6.2f%% %6.2f%% %6.2f%%  %9.3f  %9.3f\n", nreps, totalt,
         ...
         100. * toverhead / totalt, mflops, gflops);
  return (totalt);
}
...

{
  ...
  if (incx != 1) {
    ...
    ix   = 1;
    dmax = fabs(ToDouble(dx[0]));
    ix   = ix + incx;
    for (i = 1; i < n; i++) {
      if (fabs(ToDouble(dx[ix])) > dmax) {
        itemp = i;
        dmax  = fabs(ToDouble(dx[ix]));
      }
      ix = ix + incx;
    }
  } else {
    ...
    dmax  = fabs(ToDouble(dx[0]));
    for (i = 1; i < n; i++)
      if (fabs(ToDouble(dx[i])) > dmax) {
        itemp = i;
        dmax  = fabs(ToDouble(dx[i]));
      }
  }
  return (itemp);
}
